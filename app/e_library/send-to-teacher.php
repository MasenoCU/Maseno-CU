                    ...
                    ?>
                        <tr>
                            <td>
								...
								if ($database) {
									try {
                                        // Select the teacher registration collection
                                        $collection = $database->selectCollection('t_registration');
                                        $cursor = $collection->find();

                                        foreach ($cursor as $document) {
                                            ?><option value="<?php echo $document['username']; ?>">
                                            <?php echo $document['username'] . " (" . $document['idno'] . ")"; ?>
                                            </option><?php
                                        }
                                    } catch (MongoDBException $e) {
                                        echo "<span style='color: red;'><b>Error !</b> Couldn't fetch teacher data: " . $e->getMessage() . "</span>";
                                    }
								}
